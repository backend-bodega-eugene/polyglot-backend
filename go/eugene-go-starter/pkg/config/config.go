...
func Load() *Config {
	...
	cfg := &Config{
		...
		DatabaseType: getEnv("DATABASE_TYPE", "sqlx"),
	}
	log.Printf("[config] env=%s addr=%s app=%s db=%s", cfg.Env, cfg.HTTPAddr, cfg.AppName, cfg.DatabaseType)
	return cfg
}
