 ...
 * 内部后台赛事玩法赔率管理接口。
 */
@Tag(name = "内部后台赛事玩法赔率管理", description = "内部后台赛事玩法赔率分页查询、新增、修改和删除接口")
@RestController
@RequestMapping("/internal/admin/matchmarketoption")
public class InternalAdminMatchMarketOptionController {

    /**
     * 赛事玩法赔率服务。
     */
    private final MatchMarketOptionService matchMarketOptionService;

    /**
     * 创建内部后台赛事玩法赔率管理接口实例。
     *
     * @param matchMarketOptionService 赛事玩法赔率服务
     */
    public InternalAdminMatchMarketOptionController(
            MatchMarketOptionService matchMarketOptionService) {
        this.matchMarketOptionService = matchMarketOptionService;
    }
